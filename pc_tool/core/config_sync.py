...
class ConfigSync:
    ...
    def push(self, config_dict: dict, serial: str | None = None) -> bool:
        ...
        Uses delete=False + manual cleanup on Windows because NamedTemporaryFile
        with delete=True locks the file, preventing adb push from reading it.
        """
        tmp = tempfile.NamedTemporaryFile(
            mode="w", suffix=".json", delete=False, encoding="utf-8"
        )
        try:
            json.dump(config_dict, tmp, ensure_ascii=False, indent=2)
            tmp.close()
            ...
            ok = self._adb.push(tmp.name, remote, serial=serial)
            if ok:
                self._adb.broadcast(
